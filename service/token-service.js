...
class TokenService {
  generateToken(payload) {
    ...
    const accessToken = jwt.sign(payload, value.JWT_ACCESS_SECRET, { expiresIn: '30m' });
    const refreshToken = jwt.sign(payload, value.JWT_REFRESH_SECRET, { expiresIn: '30d'});
    return { accessToken, refreshToken };
  };

  validateAccessToken(token) {
    try {
      const userData = jwt.verify(token, value.JWT_ACCESS_SECRET);
      return userData;
    } catch(e) {
      return null;
    }
  ...
  validateRefreshToken(token) {
    try {
      const userData = jwt.verify(token, value.JWT_REFRESH_SECRET);
      return userData;
    } catch(e) {
      return null;
    }
